...
function countTask2 () {
    resultTask2.value = (+num1.value + +num2.value + +num3.value)/3;
}
...
function countTask3 () {
    ...
    v.value = Math.PI * + r.value * 2 * +h.value;
    s.value = 2 * Math.PI * +r.value * (+r.value + +h.value);
}
...
function countTask7 () {
   ...
   let arr =  new Array(+arrSize).fill().map((d,el) => el = Math.floor(Math.random() * 100));
   ...
   arrMax.value = max;
    ...
    arrAverage.value = average.toPrecision(3);
...
arrSource.style.cssText = `
    border: 1px solid black;
    width: 150px;
    text-align: center;
    font-size: 22px;
`;
arrNew.style.cssText = `
    border: 1px solid black;
    width: 150px;
    text-align: center;
    font-size: 22px;
